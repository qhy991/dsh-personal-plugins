...
| 自主 Workflow / Mission（只读或单文件事务） | KerSor preset → Host `kersor_evolve` → `runtime=dsh` → DSH `spawn` child | 安装记录的 K2.7、Infini-AI K3 或 DeepSeek V4 Flash route receipt、durable usage／terminal evidence、Core transaction／Host gate、`result.json` |
...
DSH-native Mission 接受 `side_effect=none|read`，也接受由 Mission authority 准入、Core 活事务绑定的单文件 `side_effect=write` capability。Planner 与只读 worker 只看见 `read/glob/grep`；事务 Execute worker 才额外获得只指向声明 artifact 的 `edit/write`。路径别名、链接、控制树、Bash、delegation、Workflow 与递归 KerSor 都被拒绝，candidate verifier 仍由 Core 在活快照内执行并决定提交或回滚。能通过实测区分当前假设的 agent 或 Host evaluator capability 可声明 `discriminating_probe: true`；该字段不授予新权限。新版 Core 在连续两次候选失败后要求先执行已准入探针，若没有探针则拒绝重复候选并暴露 capability gap。`kersor-dsh-host-rpc-v3` 允许省略 `activation_budget`：Host 会在旧版 registration-bound `llm/prepared-stream` 或当前 `llm/stream` 边界绑定安装记录的模型 route 并记录 usage；当前边界不公开精确 context window，因此只接受无显式 token 上限的 activation。若调用方显式声明正数预算，仍要求 `llm/prepared-stream` 提供 actual dispatch context window，保留 upper-bound charge 与 typed exhaustion 语义。Route integrity、Typed `DSH_CHILD_QUOTA` 与 denied mutation 的严格门禁不变。每个顶层 DSH session 仍只能调用一次 `kersor_evolve`。
...
Child execution event 的 `seq` 必须从 0 连续，fresh `turn=1` 的 step 从 1 逐一闭合，并由唯一 `turn/end` 关闭执行；清理期间只有 `session/title` metadata 可以跟在 terminal 后面，任何新的 assistant／tool／step／turn 事件仍使证据失效。Quota 终末 step 不允许 retry、output、tool 或 usage；近似 code/status、坐标漂移、failure 不匹配、result output 不一致或任一先前 step 未完整计量，都会降级为普通 incomplete `DSH_CHILD_TERMINAL_ERROR`。一般非配额 usage 只有在 lifecycle 与 Host stream ledger 一致时才完整；child deadline 使用 typed `DSH_CHILD_TIMEOUT`，`blocked`／`aborted`／`interrupted` 分别投影为 `refusal`／`aborted`／`error`。
