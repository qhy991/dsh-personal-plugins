...
const AUTHOR_STAGING_FILES = Object.freeze(['workflow.js', 'metadata.json', 'rationale.md']);
...
function normalizedObjective(value) {
    ...
    if (objective.length === 0)
        throw new Error('KerSor experiment objective must not be empty');
...
async function frozenKersorRoot() {
    ...
    if (configured !== undefined && configured.trim().length > 0) {
        if (!isAbsolute(configured))
            throw new Error('KERSOR_ROOT must be an absolute path');
        const canonical = await realpath(configured);
        if (!(await stat(canonical)).isDirectory())
            throw new Error('KERSOR_ROOT must name a directory');
        return canonical;
    }
    const python = await frozenKersorPython();
    const dshHome = process.env.DSH_HOME?.trim() || join(homedir(), '.dsh');
    const bridge = await realpath(join(dshHome, '.agent-presets', 'kersor', 'bin', 'kersor_bridge.py'));
    const output = await new Promise((resolveOutput, rejectOutput) => {
        ...
        child.once('close', (code) => {
            if (code !== 0 || size > 1024 * 1024) {
                rejectOutput(new Error(`KerSor bridge root lookup failed: ${Buffer.concat(stderr).toString('utf8').trim() || `exit ${code}`}`));
            }
            else {
    ...
    if (!isAbsolute(declared))
        throw new Error('KerSor bridge returned a non-absolute root');
...
function experimentBindings(events) {
    ...
    for (const event of events) {
        ...
        if (binding === undefined || binding.start.childSessionId !== event.data.childSessionId)
            continue;
...
function experimentClosure(checkpoint) {
    if (checkpoint.phase === 'stalled' || checkpoint.status === 'blocked')
...
async function checkpoint(ctx, session, start, status, nextAction, projection = {}) {
    ...
    if (previous !== undefined) {
        ...
        if (JSON.stringify(priorProjection) === JSON.stringify(candidate))
            return;
    }
...
function gateStatus(value) {
    if (value === 'pass' || value === 'not_required')
        return 'completed';
    if (value === 'fail')
        return 'failed';
    return 'pending';
...
function experimentStatus(phase) {
    if (phase === 'complete' || phase === 'single_run')
        return 'completed';
    if (phase === 'stalled')
        return 'blocked';
    if (phase === 'cancelled')
...
function statusProjection(meta) {
    ...
    if (value === undefined || value.kind !== 'kersor-status' || typeof value.found !== 'boolean')
        return undefined;
    ...
    return {
        ...
        projection: {
            ...(sessionDir === undefined
                ? {}
                : { kersorSessionId: sessionDir.replaceAll('\\', '/').split('/').at(-1) }),
...
function parentOf(exec) {
    if (exec.agent === undefined)
        throw new Error('KerSor experiment controls require a calling dsh agent');
...
function workspaceOf(parent) {
    ...
    if (workspace === undefined)
        throw new Error('KerSor experiment controls require a dsh workspace');
    return workspace;
}
...
function consumeKersorProtocolAction(agent, callId, action, afterEventIndex = -1) {
    const calls = agent.session.events.flatMap((event, index) => {
        if (index <= afterEventIndex)
            return [];
        if (event.type !== 'tool/call' || event.data.name !== 'kersor_protocol')
            return [];
        let value;
        try {
...
async function kersorProtocolAuthority(ctx, agent) {
    ...
    const initializationEvents = agent.session.events.filter((event) => event.type === 'kersor/session-initialized'
    ...
    const authority = await validateSessionAuthority(ctx, agent, owned.binding, sessionDir);
    const data = authority.data;
    const workspace = data.workspace;
    if (typeof workspace !== 'string') {
        throw new Error('kersor_protocol durable authority lost its workspace');
    }
    const kersorPython = historicalFileBinding(data.kersor_python, undefined, 'kersor_protocol frozen Python');
    const currentPython = await boundedFileBinding(kersorPython.path, MAX_KERSOR_PYTHON_BYTES, 'kersor_protocol frozen Python');
    ...
    const adapter = historicalFileBinding(data.adapter, undefined, 'kersor_protocol frozen adapter');
    ...
    const currentSessionConfig = await boundedFileBinding(sessionConfig.path, MAX_BASELINE_AUTHORITY_BYTES, 'kersor_protocol Session config');
    ...
    const config = await readBoundedJsonObject(sessionConfig.path, MAX_BASELINE_AUTHORITY_BYTES, 'kersor_protocol Session config');
    ...
    const state = await currentSessionStateSnapshot(sessionDir, owned.binding.start.launch, undefined, agent.id);
    return {
        controller: agent,
        sessionDir,
        workspace,
...
async function runKersorProtocolChild(ctx, authority, dispatch, signal) {
    ...
    if (execution.status === 'rejected' && disposal.status === 'rejected') {
        throw new AggregateError([execution.reason, disposal.reason], 'KerSor protocol child execution and disposal both failed');
    }
...
async function runKersorSelectionChild(ctx, authority, dispatch, decisionPath, hostGate, signal) {
    ...
    try {
        ...
        const [disposal] = await Promise.allSettled([run.dispose()]);
        if (execution.status === 'rejected' && disposal.status === 'rejected') {
            throw new AggregateError([execution.reason, disposal.reason], 'KerSor selection child execution and disposal both failed');
        }
        if (execution.status === 'rejected')
            throw execution.reason;
        if (disposal.status === 'rejected') {
        ...
        if (!state.successfulWrite) {
            throw new Error('KerSor strategy-selector completed without one Host-observed successful routing-decision write');
        }
        return run.id;
    }
    finally {
...
async function ensureKersorSelectionBoundary(authority, currentCatalog) {
    if (process.env.KERSOR_PAIR_ID?.trim()) {
        throw new Error('kersor_protocol select_workflow does not support environment-only paired routing; bind the pair identity and shared decision store to durable Session authority first');
    }
    const contextPath = kersorSelectionContextPath(authority);
    if (!await pathExists(contextPath)) {
        if (await pathExists(join(authority.sessionDir, `round-${authority.currentRound}-selection.json`))) {
    ...
    const previousCatalog = historicalFileBinding(previous.catalog, currentCatalog.path, 'previous KerSor selection context catalog');
    if (isDeepStrictEqual(previousCatalog, currentCatalog)) {
        throw new Error('KerSor select_workflow action is already consumed for the current round and unchanged workflow catalog');
    }
}
async function readKersorSelectionContext(authority, expectedCatalog) {
    ...
    const currentSelection = await boundedFileBinding(selectionPath, MAX_KERSOR_SELECTION_BYTES, 'KerSor selection context selection');
    ...
    if (decisionPath !== join(authority.sessionDir, `round-${authority.currentRound}-routing-decision.json`)) {
...
async function executeKersorProtocol(ctx, authority, action, callId, hostGate, signal) {
    if (action === 'profile') {
        ...
        const dispatch = await kersorProtocolDispatch(authority, join(authority.sessionDir, 'profile-handoff', 'context.json'), 'KerSor profile context');
        const producerSessionId = await runKersorProtocolChild(ctx, authority, dispatch, signal);
    ...
    if (action === 'select_workflow') {
        ...
        if (selectionContext.dispatch !== undefined) {
            await runKersorSelectionChild(ctx, authority, selectionContext.dispatch, selectionContext.decisionPath, hostGate, signal);
        }
        ...
        if (selectionContext.dispatch === undefined) {
            ...
            await boundedFileBinding(selectionContext.decisionPath, MAX_KERSOR_SELECTION_BYTES, 'KerSor routing decision');
        }
        outputs.push(await runKersorProtocolProcess(ctx, authority, action, [
            'bash',
    ...
    const currentAuthorContext = await boundedFileBinding(contextPath, MAX_KERSOR_PROTOCOL_CONTEXT_BYTES, 'KerSor author context');
...
async function durableAttachOrigin(ctx, parent, experimentId, suppliedLaunch) {
    ...
    const controller = { id: sourceSession.id, session: sourceSession };
    const initialization = await validateSessionInitialization(controller, origin.binding);
    if (sourceSession.events.some(event => event.type === 'kersor/session-authority-transferred')) {
    ...
    if (last === undefined)
        throw new Error('KerSor attach source controller has no durable authority log');
...
function exactAttachedStartCall(parent, start, intent) {
    const startEvents = parent.events.filter(event => event.type === 'kersor/experiment-start'
...
function sourceAuthorityForIntent(ctx, start) {
    ...
    const sourceBindings = experimentBindings(sourceParent.events).filter(candidate => candidate.start.experimentId === start.experimentId
    ...
    return {
        parent: sourceParent,
        controller: { id: sourceSession.id, session: sourceSession },
        intent,
    };
}
async function ensureSourceAuthorityTransfer(ctx, targetParent, start) {
    ...
    if (sourceBinding === undefined)
        throw new Error('attached KerSor source binding disappeared');
    const projected = agentWithAuthorityEvents(controller, prePrefix);
    const initialization = await validateSessionInitialization(projected, sourceBinding, intent.session_dir);
    ...
    const importCommitted = targetSession?.events.some(event => event.type === 'kersor/session-authority-imported'
        ...
        async execute(args, exec) {
            ...
            try {
                ...
                if (controller !== undefined) {
                    if (!controller.events.some(event => event.type === 'kersor/session-authority-imported'
...
function experimentControllerAgent(ctx, descendant) {
    ...
    while (child.header.parentSession !== undefined) {
        ...
        if (binding !== undefined) {
            return {
                controller: { id: child.id, session: child },
                binding,
            };
        }
        child = parent;
...
async function sessionRootChildren(workspace) {
    ...
    for (const entry of entries) {
        if (await isCanonicalSessionRootEntry(root, entry))
...
async function beginSessionSetup(exec, binding, hostGate) {
    ...
    if (agent === undefined)
        return 'KerSor Session setup requires its controller Agent';
    const callId = exec.callId;
    const argumentsValue = exec.arguments;
    const command = bashCommand(argumentsValue);
    if (command === undefined)
        return undefined;
    if (!command.includes('setup-session.sh'))
        return undefined;
    ...
    if (cwd === undefined)
        return 'KerSor Session setup requires a controller workspace';
    ...
    if (agent.session.events.some(event => event.type === 'kersor/session-initialized'
...
async function sessionInitializationData(controller, experimentId, setupCallId, sessionDir, launch, freshSession, kersorRoot, kersorPython) {
    ...
    const config = await readBoundedJsonObject(owner.sessionConfig.path, MAX_BASELINE_AUTHORITY_BYTES, 'initialized Session config');
    const workflowDir = config.workflow_dir;
    if (typeof workflowDir !== 'string' || !isAbsolute(workflowDir)
...
async function finishSessionSetup(ctx, state) {
    ...
    if (typeof sessionDir !== 'string')
        throw new Error('Session initialization lost its Session path');
...
async function validateSessionInitialization(agent, binding, sessionDir) {
    const events = agent.session.events.filter(event => event.type === 'kersor/session-initialized'
    ...
    const kersorPython = historicalFileBinding(data.kersor_python, undefined, 'Session initialization KerSor Python');
    ...
    const currentAdapter = await boundedFileBinding(adapter.path, MAX_DSH_WORKFLOW_SOURCE_BYTES, 'current KerSor setup-session.sh adapter');
...
async function validateSessionAuthority(ctx, agent, binding, sessionDir) {
    ...
    const imported = agent.session.events.filter(event => event.type === 'kersor/session-authority-imported'
        && event.data.experiment_id === binding.start.experimentId);
    ...
    if (parent === undefined)
        throw new Error('attached KerSor parent Session is unavailable');
    ...
    const sourceBindings = sourceParent === undefined
        ? []
        : experimentBindings(sourceParent.events).filter(candidate => candidate.start.experimentId === binding.start.experimentId
            && candidate.start.origin === 'created');
    ...
    const sourceController = agentWithAuthorityEvents({ id: sourceSession.id, session: sourceSession }, prefix);
...
async function dispatchAuthorityAgent(ctx, agent, runDir) {
    ...
    if (binding === undefined)
        throw new Error('dispatch custody lost its Experiment binding');
...
function dispatchProducerSpec(argumentsValue) {
    ...
    if (lines[0] !== DISPATCH_PRODUCER_MARKER)
        return undefined;
...
async function durableDispatchProducerCallIds(agent, runDir) {
    ...
    for (const event of agent.session.events) {
        ...
        if (spec === undefined || !isAbsolute(spec.runDir))
            continue;
...
async function durableBaselineCallIds(agent, phase, sessionDir, workspace, launch) {
    ...
    for (const event of agent.session.events) {
        ...
        if (bashCommand(argumentsValue) === canonical)
            matches.push(event.data.callId);
    }
    return matches;
}
...
async function durableWorkflowCallIds(agent, runDir) {
    const matches = [];
    for (const event of agent.session.events) {
        if (event.type !== 'tool/call' || event.data.name !== 'kersor_workflow')
            continue;
        let argumentsValue;
        try {
            argumentsValue = JSON.parse(event.data.arguments);
        ...
        if (typeof expDir !== 'string' || !isAbsolute(expDir))
            continue;
...
async function readDshWorkflowEnvelope(runDir) {
    ...
    if (identity.isSymbolicLink())
        throw new Error('Workflow envelope dsh-workflow.json must not be a symlink');
    if (!identity.isFile())
        throw new Error('Workflow envelope dsh-workflow.json must be a regular file');
...
function balancedObjectEnd(source, openIndex) {
    if (source[openIndex] !== '{')
        throw new Error('Workflow meta object must start with {');
    let depth = 0;
    let state = 'code';
    let quote = '';
...
function projectWorkflowMeta(meta) {
    ...
    if (meta.phases !== undefined) {
        if (!Array.isArray(meta.phases))
            throw new Error('selected Workflow meta.phases must be an array');
        phases = meta.phases.map((value, index) => {
            ...
            if (dropped.length > 0)
                droppedPhaseFields.push({ index, fields: dropped });
            return {
                title: phase.title,
...
function canonicalWorkflowSource(source) {
    ...
    if (match === undefined)
        throw new Error('selected Workflow meta declaration is missing');
    const open = match.index + match[0].length;
    if (source[open] !== '{')
        throw new Error('selected Workflow meta must use an inline object literal');
...
async function readBoundedRegularFile(path, limit, label) {
    ...
    if (await realpath(path) !== path)
        throw new Error(`${label} contains a symlink or path alias`);
...
function committedSelectedWorkflow(selection, selectionName) {
    ...
    if (selected === undefined || typeof selected.name !== 'string'
        ...
        throw new Error(`${selectionName} must contain one committed selection whose attempt_plan commit and non-pending routing decision bind selected_workflow.name`);
    }
    return selected;
}
async function hostSelectedWorkflowName(runDir) {
    ...
    const catalog = await readBoundedJsonObject(catalogPath, MAX_KERSOR_CATALOG_BYTES, 'workflow-catalog.json');
    ...
    if (normalizedSha256(catalogEntry.workflow_content_hash, 'workflow-catalog.json selected workflow_content_hash') !== selectedHash) {
...
async function validateWorkflowSourceBinding(runDir, envelope) {
    ...
    const catalogHash = normalizedSha256(catalogEntry.workflow_content_hash, 'workflow-catalog.json selected workflow_content_hash');
    ...
    if (selected.candidate_type === 'authored') {
        ...
        const expected = join(sessionDir, 'workflow-authoring', 'proposals', selected.directory, 'workflow.js');
    ...
    if (normalizedSha256(compatibility.workflow_sha256, 'dsh-compatibility.json workflow_sha256') !== sourceHash) {
    ...
    const projectedMetaHash = createHash('sha256')
        ...
        .digest('hex');
    if (normalizedSha256(source.original_meta_sha256, 'dsh-workflow.json source.original_meta_sha256') !== originalMetaHash
        || normalizedSha256(compatibility.original_meta_sha256, 'dsh-compatibility.json original_meta_sha256') !== originalMetaHash
        || normalizedSha256(source.projected_meta_sha256, 'dsh-workflow.json source.projected_meta_sha256') !== projectedMetaHash
        || normalizedSha256(compatibility.projected_meta_sha256, 'dsh-compatibility.json projected_meta_sha256') !== projectedMetaHash) {
    ...
    const persistedArgs = await readBoundedJsonObject(argsPath, MAX_DSH_DISPATCH_ARGS_BYTES, 'dispatch-args.json');
    ...
    if (normalizedSha256(source.args_sha256, 'dsh-workflow.json source.args_sha256') !== argsHash
        || normalizedSha256(compatibility.args_sha256, 'dsh-compatibility.json args_sha256') !== argsHash) {
    ...
    if (bodyHash !== actualBodyHash || normalizedSha256(compatibility.body_sha256, 'dsh-compatibility.json body_sha256') !== bodyHash) {
    ...
    const effectiveHash = createHash('sha256')
        ...
        .digest('hex');
    if (normalizedSha256(source.effective_script_sha256, 'dsh-workflow.json source.effective_script_sha256') !== effectiveHash
        || normalizedSha256(compatibility.effective_script_sha256, 'dsh-compatibility.json effective_script_sha256') !== effectiveHash) {
...
async function runGitCommand(root, args) {
    return new Promise((resolveCommand, rejectCommand) => {
        ...
        child.stderr.on('data', (chunk) => {
            ...
            if (stderrBytes <= MAX_GIT_COMMAND_OUTPUT_BYTES)
                stderr.push(chunk);
        });
...
async function candidateGitRoot(projectRoot) {
    ...
    if (result.code !== 0) {
        ...
        if (/\bnot a git repository\b/i.test(diagnostic))
            return undefined;
    ...
    if (!isAbsolute(declared))
        throw new Error('candidate ownership git root is not absolute');
    const canonical = await realpath(declared);
    if (canonical !== declared)
        throw new Error('candidate ownership git root is not canonical');
    return canonical;
}
async function currentProtectedFiles(projectRoot, kernelPath, gitRoot) {
    ...
    for (const path of protectedRoots)
        await collectProtectedFiles(path, selected);
    ...
    for (const path of [...selected].sort()) {
        result[relativeOwnershipPath(projectRoot, path, 'protected file')] = await sha256RegularFile(path, `protected file ${path}`);
    }
    return result;
}
function ownershipMapDifference(expected, current) {
    for (const path of [...new Set([...Object.keys(expected), ...Object.keys(current)])].sort()) {
        if (!isDeepStrictEqual(expected[path], current[path]))
            return path;
    }
    return undefined;
}
async function currentWorktreeSnapshot(sessionDir, gitRoot) {
    ...
    const staged = await requiredGitBytes(gitRoot, ['diff', '--cached', '--binary', '--no-ext-diff', '--', '.']);
    const untrackedRaw = await requiredGitBytes(gitRoot, ['ls-files', '--others', '--exclude-standard', '-z']);
    ...
    for (const path of untrackedRaw.toString('utf8').split('\0').filter(Boolean).sort()) {
        ...
        if (identity?.isFile())
            untracked[path] = await sha256RegularFile(await realpath(candidate), `untracked file ${path}`);
    }
    return {
        git_root: gitRoot,
...
async function currentSessionStateSnapshot(sessionDir, launch, expectedRound, expectedSessionId) {
    ...
    if (!Number.isSafeInteger(state.current_round) || state.current_round < 1) {
    ...
    if (!Number.isSafeInteger(state.stall_count) || state.stall_count < 0
...
async function baselineOwner(agent, sessionDirValue, launch) {
    const cwd = agent.session.header.cwd;
    if (cwd === undefined)
        throw new Error('baseline custody requires a controller workspace');
...
async function baselineAuthority(owner) {
    const current = await baselineOwner({ session: { header: { cwd: owner.workspace } } }, owner.sessionDir, owner.launch);
...
async function validateBaselineWitness(authority) {
    ...
    if (witness === undefined || !hasExactKeys(witness, BASELINE_WITNESS_KEYS)
        ...
        || session === undefined || !hasExactKeys(session, ['id', 'dir', 'started_at', 'config_sha256'])
    ...
    if (source.kernel_path !== authority.kernel.path
        ...
        throw new Error(`baseline witness kernel binding changed: ${relative(authority.workspace, authority.kernel.path).split(sep).join('/')}`);
    }
...
async function validateDispatchCustody(agent, runDir, envelope) {
    const source = record(envelope.source);
    if (source === undefined)
        throw new Error('dsh-workflow.json lacks dispatch custody source');
    const sessionDir = dirname(runDir);
    const round = Number.parseInt(basename(runDir).slice('run-'.length), 10);
    ...
    if (!isDeepStrictEqual(source.dispatch_args_producer, producer)
        ...
        || normalizedSha256(source.dispatch_args_producer_receipt_sha256, 'dsh-workflow.json producer receipt hash') !== producerHash) {
    ...
    if (!isDeepStrictEqual(compatibility.dispatch_args_producer, producer)
        ...
        || normalizedSha256(compatibility.dispatch_args_producer_receipt_sha256, 'dsh-compatibility.json producer receipt hash') !== producerHash) {
    ...
    const producerReceiptBinding = transformation === undefined
        ? undefined
        : receiptFileBinding(transformation, 'producer_receipt', producerPath, 'dispatch transformation receipt');
    ...
    const producerCallIndex = agent.session.events.findIndex(event => event.type === 'tool/call' && event.data.name === 'subagent'
    ...
    if (!isDeepStrictEqual(source.dispatch_args_transformation, transformation)
        ...
        || normalizedSha256(source.dispatch_args_transformation_receipt_sha256, 'dsh-workflow.json transformation receipt hash') !== transformationHash
        || compatibility.dispatch_args_transformation_receipt_source !== transformPath
        || normalizedSha256(compatibility.dispatch_args_transformation_receipt_sha256, 'dsh-compatibility.json transformation receipt hash') !== transformationHash
    ...
    if (source.dispatch_args_provenance_path !== provenancePath
        || normalizedSha256(source.dispatch_args_provenance_sha256, 'dsh-workflow.json dispatch provenance hash') !== provenanceHash
        || compatibility.dispatch_args_provenance_source !== provenancePath
        || normalizedSha256(compatibility.dispatch_args_provenance_sha256, 'dsh-compatibility.json dispatch provenance hash') !== provenanceHash) {
...
async function validateCandidateOwnershipSealFile(ctx, agent, runDir, envelope, custody, launch) {
    ...
    for (const [name, rawEvidence] of Object.entries(files)) {
        ...
        if (normalizedSha256(evidence.file_sha256, `candidate ownership seal ${name} hash`) !== actualHash) {
...
async function validateCandidateOwnershipSeal(ctx, custodyAgent, runDir, envelope, custody, workflowCallId, launch, workflowAgent = custodyAgent) {
    ...
    const transformationEvent = custodyAgent.session.events.find(candidate => candidate.type === 'kersor/dispatch-args-transformed'
        && isDeepStrictEqual(candidate.data, custody.transformation));
    const workflowCall = workflowAgent.session.events.find(candidate => candidate.type === 'tool/call' && candidate.data.callId === workflowCallId
        && candidate.data.name === 'kersor_workflow');
    const sealCall = custodyAgent.session.events.find(candidate => candidate.type === 'tool/call' && candidate.data.callId === data.seal_call_id
        && candidate.data.name === 'bash');
    const producerCall = custodyAgent.session.events.find(candidate => candidate.type === 'tool/call'
        && candidate.data.callId === custody.producer.producer_call_id
        && candidate.data.name === 'subagent');
    const producerEvent = custodyAgent.session.events.find(candidate => candidate.type === 'kersor/dispatch-args-produced'
    ...
    if (!sameController) {
        ...
        const imported = await validateSessionAuthority(ctx, workflowAgent, workflowBinding, sessionDir);
...
function createSealedWorkflow(ctx, hostGate) {
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            if (result === undefined)
                throw new Error('native workflow returned no Host result');
            if (result.isError)
    ...
    const visit = (value, key) => {
        if (typeof value === 'string') {
            if (key === 'file_path' || key === 'filePath' || key === 'path'
                || key === 'directory' || key === 'root' || key === 'cwd')
                paths.push(value);
            return;
        }
        if (Array.isArray(value)) {
        ...
        if (valueRecord === undefined)
            return;
        for (const [childKey, child] of Object.entries(valueRecord))
...
function matchesStaticGlob(value, pattern) {
    ...
    for (let index = 0; index < pattern.length; index += 1) {
        ...
        if (character === '[') {
            ...
            if (end < 0)
                throw new Error('unterminated static glob class');
            const body = pattern.slice(index + 1, end);
            if (body.length === 0)
                throw new Error('empty static glob class');
...
async function authorAuthorityForAgent(ctx, agent) {
    ...
    if (controller === undefined || binding === undefined
        || binding.start.origin !== 'created'
        || binding.start.launch.workflow_authoring_budget === 0)
        return undefined;
    const events = controller.session.events.filter((event) => event.type === 'kersor/session-initialized'
        && event.data.experiment_id === binding.start.experimentId);
    const event = events[0];
    if (events.length === 0 || event === undefined)
        return undefined;
...
async function sameRegularFileIdentity(candidate, protectedPaths) {
    ...
    if (candidateStat === undefined || !candidateStat.isFile())
        return false;
    const protectedStats = await Promise.all(protectedPaths.map(path => stat(path).catch(() => undefined)));
...
async function targetsAuthorStaging(authority, agent, filePath) {
    ...
    if (pathInside(authority.stagingDir, lexicalTarget))
        return true;
    ...
    return (realStaging !== undefined && realTarget !== undefined
        ...
        || await sameRegularFileIdentity(rawTarget, AUTHOR_STAGING_FILES.map(name => join(authority.stagingDir, name)));
}
    ...
    while (pending.length > 0) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isSymbolicLink()) {
                ...
                if (target === undefined || !pathRelated(staging, target))
                    continue;
                ...
                try {
                    if (candidates.some(candidate => matchesStaticGlob(candidate, filter)))
                        return true;
                }
                catch {
                    return true;
                ...
                && await sameRegularFileIdentity(path, AUTHOR_STAGING_FILES.map(name => join(staging, name)))) {
                const candidates = [
                    path,
...
async function authorPathToolTargetsStaging(authority, agent, toolName, argumentsValue) {
    ...
    if ((await Promise.all(paths.map(path => targetsAuthorStaging(authority, agent, path)))).some(Boolean))
        return true;
    ...
    if (pathInside(root, staging)
        && searchFilterMatchesAuthorStaging(root, staging, filter))
        return true;
...
async function shellWordTargetsAuthorStaging(word, cwd, authority) {
    ...
    if (/[*?[]/u.test(candidate)) {
        if (known.some(path => matchesStaticGlob(path, lexical)))
            return true;
        const expanded = await boundedShellGlob(raw);
        if (expanded === undefined)
            return true;
        for (const path of expanded) {
            if (pathRelated(authority.stagingDir, path))
                return true;
            const realPath = await realMutationTarget(path);
            const realStaging = await realpath(authority.stagingDir).catch(() => undefined);
            if (realPath !== undefined && realStaging !== undefined
                && pathRelated(realStaging, realPath))
                return true;
        }
    }
    if (pathRelated(authority.stagingDir, lexical))
        return true;
    ...
    return (realStaging !== undefined && realCandidate !== undefined
        ...
        || await sameRegularFileIdentity(raw, AUTHOR_STAGING_FILES.map(name => join(authority.stagingDir, name)));
}
async function boundedShellGlob(pattern) {
    ...
    while (pending.length > 0) {
        ...
        for (const entry of entries) {
            ...
            const path = join(directory, entry.name);
            try {
                if (matchesStaticGlob(path, pattern))
                    matched.push(path);
            }
            catch {
                return undefined;
...
async function shellWordIsAuthorGateScript(word, cwd, authority) {
    ...
    if (/[*?[]/u.test(candidate)) {
        try {
            if (scripts.some(path => matchesStaticGlob(path, lexical)))
                return true;
        }
        catch {
            return true;
        ...
        for (const path of expanded) {
            const realPath = await realMutationTarget(path);
            if (realPath !== undefined && scripts.includes(realPath))
                return true;
        }
    }
    if (scripts.includes(lexical))
        return true;
...
async function shellInvokesAuthorGate(executableWord, executable, operands, cwd, authority) {
    if (await shellWordIsAuthorGateScript(executableWord, cwd, authority))
        return true;
    ...
    if (realExecutable === wrapper
        && operands.some(operand => basename(operand) === 'seal-author-handoff.py'))
        return true;
    ...
    for (const target of targets) {
        if (await shellWordIsAuthorGateScript(target, cwd, authority))
            return true;
    }
    if (['bash', 'sh', 'zsh'].includes(executable)) {
        ...
        if (script !== undefined && tool !== undefined) {
            ...
            if (realScript === wrapper && basename(tool) === 'seal-author-handoff.py')
                return true;
        }
    }
    return false;
...
async function authorBashWorkdir(argumentsValue, authority, agent) {
    ...
    if (authored === undefined)
        return { cwd: workspace, invalid: false, staging: false };
    if (typeof authored !== 'string')
        return { cwd: workspace, invalid: true, staging: false };
...
async function isTrustedKersorHelperInvocation(argumentsValue, authority) {
    ...
    if (custodyScripts.map(path => basename(path)).includes(scriptName)
        || await sameRegularFileIdentity(realScript, custodyScripts))
        return false;
    if (operands[0] !== 'save')
        return true;
...
async function bashTouchesAuthorStaging(argumentsValue, authority, agent, stagingAccess = true) {
    ...
    if (command === undefined)
        return true;
    const workdir = await authorBashWorkdir(argumentsValue, authority, agent);
    if (workdir.invalid || stagingAccess && workdir.staging)
        return true;
    ...
    const inspect = async (source, inheritedCwd, inheritedVariables, depth) => {
        ...
        for (const words of shellCommands(tokens)) {
            ...
            if (redirections.dynamic && stagingAccess)
                return true;
            ...
            for (; index < invocationWords.length; index += 1) {
                ...
                if (name === undefined || assignmentValue === undefined)
                    return true;
            ...
            for (const target of redirections.targets) {
                ...
                if (stagingAccess
                    && await shellWordTargetsAuthorStaging(expanded, cwd, authority))
                    return true;
            }
            if (index >= invocationWords.length)
                continue;
            const unexpandedExecutable = invocationWords.at(index);
            if (unexpandedExecutable === undefined)
                return true;
            ...
            for (const word of invocationWords.slice(index + 1)) {
                ...
                if (expanded === undefined) {
                    if (shellExecutableUsesPathOperands(initialExecutable)
                        || SHELL_EXECUTION_WRAPPERS.has(initialExecutable))
                        return true;
                    continue;
                }
                operands.push(expanded);
            ...
            if (await shellInvokesAuthorGate(effectiveExecutableWord, executable, effectiveOperands, cwd, authority))
                return true;
            ...
            if (['bash', 'sh', 'zsh'].includes(executable)) {
                ...
                if (commandIndex >= 0) {
                    ...
                    if (payload === undefined
                        || await inspect(payload, cwd, effectiveVariables, depth + 1))
                        return true;
                }
            }
            else if (executable === 'eval') {
                if (effectiveOperands.length === 0
                    || await inspect(effectiveOperands.join(' '), cwd, effectiveVariables, depth + 1))
                    return true;
            }
            if (!shellExecutableUsesPathOperands(executable))
                continue;
            if (!stagingAccess)
                continue;
            ...
            for (const candidate of candidates) {
                if (await shellWordTargetsAuthorStaging(candidate, cwd, authority))
                    return true;
            }
        }
        return false;
...
function authorSealEvent(authority) {
    const events = authority.controller.session.events.filter((event) => event.type === 'kersor/author-handoff-sealed'
        && event.data.session_dir === authority.sessionDir);
    if (events.length > 1)
        throw new Error('author handoff has duplicate durable Host seals');
    return events[0];
}
function authorProducedEvent(authority) {
    const events = authority.controller.session.events.filter((event) => event.type === 'kersor/author-produced'
        && event.data.session_dir === authority.sessionDir);
    if (events.length > 1)
        throw new Error('author handoff has duplicate durable Host producers');
    return events[0];
}
function authorSaveEvents(authority) {
    return authority.controller.session.events.filter((event) => event.type === 'kersor/author-save-attempted'
        && event.data.session_dir === authority.sessionDir);
}
function durableAuthorCommitCallIds(controller, action, afterSeq) {
    ...
    for (const event of controller.session.events) {
        if (event.seq <= afterSeq
            || event.type !== 'tool/call' || event.data.name !== 'kersor_author_commit')
            continue;
        try {
...
async function sealAuthorHandoff(ctx, authority, callId, signal) {
    ...
    const currentContext = await boundedFileBinding(produced.data.author_context.path, MAX_KERSOR_PROTOCOL_CONTEXT_BYTES, 'KerSor author context');
...
async function saveAuthorHandoff(ctx, authority, callId, signal) {
    ...
    if (seal === undefined)
        throw new Error('authored Proposal save requires its durable Host author seal');
...
async function validateRefreshedAuthorCatalog(catalogPath, proposal) {
    const catalog = await readBoundedJsonObject(catalogPath, MAX_KERSOR_CATALOG_BYTES, 'refreshed workflow-catalog.json');
...
function nextBaselinePhase(events, sessionDir) {
    const phases = ['initialized', 'recorded', 'verified'];
    let previousIndex = -1;
...
async function baselineCommandSpec(command, workspace, launch) {
    ...
    for (const variant of variants) {
        if (!command.startsWith(variant.head) || !command.endsWith(variant.tail))
            continue;
        ...
        if (sessionDir === undefined
            || command !== await canonicalBaselineCommand(variant.phase, sessionDir, workspace, launch))
            return undefined;
...
async function isRoundSynthesisArtifact(agent, filePath) {
    ...
    if (roundSynthesisPathParts(lexicalWorkspace, lexicalTarget) !== undefined)
        return true;
    if (!(await pathExists(lexicalTarget)))
        return false;
    try {
        const [realWorkspace, realTarget] = await Promise.all([
...
function hasRoundSynthesisMutation(command) {
    ...
    if (!new RegExp(artifact, 'i').test(command))
        return false;
...
async function isRoutingDecisionArtifact(agent, filePath) {
    ...
    const lexicalWorkspace = resolve(workspace);
    const lexicalTarget = isAbsolute(filePath) ? resolve(filePath) : resolve(lexicalWorkspace, filePath);
    if (routingDecisionPathParts(lexicalWorkspace, lexicalTarget) !== undefined)
        return true;
    const [realWorkspace, realTarget] = await Promise.all([
        realpath(lexicalWorkspace).catch(() => undefined),
        realMutationTarget(lexicalTarget),
...
function hasRoutingDecisionMutation(command) {
    ...
    if (!new RegExp(artifact, 'iu').test(command))
        return false;
...
function manuallyAdvancesRound(command) {
    if (!/\bkersor-state\.sh\b/i.test(command))
        return false;
...
async function isExistingRunOutput(agent, filePath) {
    ...
    if (!(await pathExists(lexicalTarget)))
        return false;
    if (runPathParts(lexicalWorkspace, lexicalTarget, true) !== undefined)
        return true;
    try {
        const [realWorkspace, realTarget] = await Promise.all([
            realpath(lexicalWorkspace),
...
async function bashMutatesExistingRunOutput(agent, command) {
    if (!/output\.json/i.test(command) || !hasRunOutputMutation(command))
        return false;
    ...
    for (const candidate of candidates) {
        if (await isExistingRunOutput(agent, candidate))
            return true;
    }
    return false;
}
...
function dispatchRuntimeAuthority(authority, sessionDir) {
    const initialized = authority.authorityEvents.filter(event => event.type === 'kersor/session-initialized'
...
async function beginDispatchProducer(ctx, agent, callId, argumentsValue, launch, hostGate) {
    ...
    for (const name of [DISPATCH_PRODUCER_RECEIPT, DISPATCH_TRANSFORMATION_RECEIPT]) {
        if (await pathExists(join(runDir, name)))
            return `dispatch producer Host receipt already exists: ${join(runDir, name)}`;
    }
...
async function finishDispatchProducer(ctx, state, resultValue, signal) {
    ...
    if (!isDeepStrictEqual([...state.successfulWrites].sort(), ['dispatch-args-provenance.json', 'dispatch-args.json'])) {
...
async function transformDispatchArgs(ctx, state, producer, signal) {
    ...
    const persistedProducer = JSON.parse(receiptBytes.toString('utf8'));
...
function baselineDataMatchesAuthority(data, authority, controllerSessionId) {
    if (!nonWhitespaceToken(data.call_id))
        return false;
...
async function baselinePhaseReceipt(agent, phase, authority) {
    ...
    const eventIndex = agent.session.events.indexOf(event);
    const callIndex = agent.session.events.findIndex(event => event.type === 'tool/call' && event.data.callId === receipt.call_id
...
async function validateBaselineCustody(ctx, agent, sessionDir, launch) {
    ...
    if (protectedFiles === undefined || worktree === undefined
        || Object.values(protectedFiles).some(value => typeof value !== 'string' || !/^[0-9a-f]{64}$/u.test(value))) {
    ...
    const currentProtected = await currentProtectedFiles(authority.workspace, authority.kernel.path, gitRoot);
...
async function beginBaselineWitness(ctx, agent, callId, command, launch, binding, hostGate) {
    if (!command.includes('baseline-witness.py'))
        return undefined;
    if (launch === undefined) {
    ...
    const cwd = agent.session.header.cwd;
    if (cwd === undefined)
        return 'baseline custody requires a controller workspace';
    const workspace = await realpath(resolve(cwd));
    ...
    if (agent.session.events.some(event => (event.type === 'kersor/dispatch-args-produced'
    ...
    if (spec.phase === 'initialized') {
        if (await pathExists(testMethodPath) || await pathExists(witnessPath)
            || agent.session.events.some(event => event.type === 'kersor/baseline-initialized'
                && event.data.session_dir === owner.sessionDir)) {
        ...
        const authority = await baselineAuthority(owner);
        const previousPhase = spec.phase === 'recorded' ? 'initialized' : 'recorded';
...
async function beginCandidateOwnershipSeal(agent, callId, command, hostGate) {
    ...
    const priorEvents = agent.session.events.filter(event => event.type === 'kersor/candidate-ownership-sealed'
...
export function apply(ctx) {
    const workflowHostGate = {
        authorizedNativeCallIds: new Set(),
        consumedRuns: new Set(),
        selectionChildren: new Map(),
        producerCalls: new Map(),
        producerChildren: new Map(),
        candidateSealCalls: new Map(),
        baselineCalls: new Map(),
        setupCalls: new WeakMap(),
    ...
    ctx.on('bash/sandbox-escalation', (exec, escalation) => {
        ...
        if (setup?.controller === exec.agent)
            escalation.suppress();
    });
    ...
    ctx.on('tools/pre-execute', async (exec, next) => {
        ...
        const transferredBy = ctx.sessions.list().flatMap(session => session.events.filter((event) => event.type === 'kersor/experiment-start'
            && event.data.origin === 'attached'
        ...
        if (owned?.binding.start.origin === 'attached'
            && !agent.session.events.some(event => event.type === 'kersor/session-authority-imported'
                && event.data.experiment_id === owned.binding.start.experimentId)
        ...
        if (owned !== undefined) {
            ...
            if (authorAuthority !== undefined && authorPathTools.has(exec.name)) {
                if (await authorPathToolTargetsStaging(authorAuthority, agent, exec.name, exec.arguments)) {
                    ...
                    const exactSealedRead = seal !== undefined && exec.name === 'read'
                        ...
                        && filePath !== undefined
                        && AUTHOR_STAGING_FILES.some(name => filePath === join(authorAuthority.stagingDir, name));
            ...
            if (exec.name === 'bash') {
                ...
                if (command !== undefined && authorAuthority !== undefined
                    && await authorBashEnvelopeTargetsStaging(exec.arguments, authorAuthority, agent)) {
                    return {
                        kind: 'deny',
        ...
        if (selectionChild !== undefined && authorPathTools.has(exec.name)
            ...
            if (exec.name !== 'write' || filePath === undefined
                || !await isExactSelectionDecisionTarget(agent, filePath, selectionChild.decisionPath)) {
                return {
                    kind: 'deny',
    ...
    ctx.on('tools/post-execute', async (exec, result, next) => {
        ...
        if (exec.agent !== undefined && exec.name === 'write'
            ...
            if (selection !== undefined && filePath !== undefined
                && await isExactSelectionDecisionTarget(exec.agent, filePath, selection.decisionPath)) {
        ...
        if (baselineState !== undefined) {
            try {
                ...
                if (failure !== undefined) {
                    return { kind: 'block', feedback: failure };
                }
        ...
        if (decision.kind !== 'accept' || result.isError || exec.agent === undefined)
            return decision;
        if (exec.name !== 'kersor_status')
            return decision;
        const owned = controllerBinding(ctx, exec.agent);
        if (owned === undefined)
            return decision;
        if (owned.binding.closure !== undefined)
            return { ...decision, concludesTurn: true };
    ...
    ctx.on('subagent/end', (info) => {
        for (const session of ctx.sessions.list()) {
            ...
            if (binding === undefined || binding.closure !== undefined)
                continue;
            reportAsync(ctx, checkpoint(ctx, session, binding.start, 'waiting', info.stopReason === 'completed'
