...
function applyExperimentEvent(trace, event, fail) {
    ...
    if (event.type === 'kersor/experiment-start') {
        if (trace.has(experimentId))
            fail(`kersor/experiment-start repeats experiment ${experimentId}`);
        ...
        if (objective.length > 4_000)
            fail('kersor/experiment-start objective exceeds 4000 characters');
        if (data.origin !== 'created' && data.origin !== 'attached')
            fail('kersor/experiment-start origin is invalid');
        if (typeof data.freshSession !== 'boolean')
            fail('kersor/experiment-start freshSession must be a boolean');
    ...
    if (current === undefined)
        fail(`kersor/experiment-checkpoint has no start for ${experimentId}`);
    ...
    if (!STATUSES.has(data.status))
        fail('kersor/experiment-checkpoint status is invalid');
    if (!Array.isArray(data.steps))
        fail('kersor/experiment-checkpoint steps must be an array');
    ...
    for (const candidate of data.steps) {
        ...
        if (stepIds.has(id))
            fail(`kersor/experiment-checkpoint repeats step ${id}`);
        stepIds.add(id);
        if (!STEP_STATUSES.has(step.status))
            fail(`kersor/experiment-checkpoint step ${id} status is invalid`);
    }
...
const install = Object.assign((ctx, fail) => {
    ...
    ctx.on('internal/dispatch', (_mode, eventName, args) => {
        ...
        if (!isExperimentEvent(event))
            return;
        const trace = new Map([...(traces.get(session) ?? new Map())]
