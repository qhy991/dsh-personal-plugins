...
const DEFAULT_STOP_GRACE_MS = 3e3;
...
let KersorService = (() => {
	...
	return class KersorService extends _classSuper {
		static {
			...
			__esDecorate(this, null, _listActive_decorators, {
				kind: "method",
				name: "listActive",
				static: false,
				private: false,
		...
		static Config = Schema.object({
			...
			tasks: Schema.array(Schema.object({
				...
				mission: Schema.string().required(),
				runtimeConfig: Schema.string()
			})).min(1).required(),
			credentialRefs: Schema.array(Schema.string()).default([]),
			env: Schema.dict(Schema.string()).default({}),
			maxOutputBytes: Schema.number().step(1).min(1).default(DEFAULT_MAX_OUTPUT_BYTES),
			stopGraceMs: Schema.number().step(1).min(1).default(DEFAULT_STOP_GRACE_MS)
		});
		...
		constructor(ctx, config) {
			...
			for (const task of config.tasks) {
				...
				this.tasks.set(task.id, {
					...
					...task.runtimeConfig === void 0 ? {} : { runtimeConfig: absolute(task.runtimeConfig, `runtimeConfig for task ${JSON.stringify(task.id)}`) }
				});
			}
		}
		...
		async *[Service.init]() {
			...
			await Promise.all([...this.tasks.values()].flatMap((task) => [access(task.mission), ...task.runtimeConfig === void 0 ? [] : [access(task.runtimeConfig)]]));
			yield async () => {
				this.stopping = true;
		...
		*/
		async start(taskId) {
			...
			const argv = [
				await this.ctx.subprocess.resolveExecutable(this.python, explicitEnv),
				this.runner,
				"--session",
				routing.session,
				"--mission",
				...
				"--project-root",
				routing.workspace,
				...task.runtimeConfig === void 0 ? [] : ["--runtime-config", task.runtimeConfig]
			];
			...
			const ref = {
				...
				runId,
				runDir,
				startedTs: (/* @__PURE__ */ new Date()).toISOString(),
				pid: handle.pid
			};
			const owned = {
				ref,
		...
		*/
		...
		async resolveEnvironment() {
			...
			for (const ref of this.credentialRefs) {
				...
				if (resolved === void 0) throw new Error(`kersor: credential ${JSON.stringify(ref)} is not configured`);
		...
		finish(launch, failure) {
			...
			if (failure !== void 0) this.ctx.logger.warn("kersor: launcher for %s ended with %s", launch.ref.runId, failure);
...
async function readMissionRouting(mission) {
	...
	if (typeof value !== "object" || value === null || Array.isArray(value)) throw new Error(`kersor: Mission must be a JSON object: ${mission}`);
	const record = value;
	if (record.contract_version !== "kersor-mission-v1") throw new Error(`kersor: only kersor-mission-v1 is launchable: ${mission}`);
	...
	if (runtime !== "codex" && runtime !== "pi" && runtime !== "kernelowl") throw new Error(`kersor: unsupported Mission runtime in ${mission}`);
	return {
		workspace,
		session,
		runtime
...
function contractPath(value, field, base, mission) {
	if (typeof value !== "string" || value.length === 0) throw new Error(`kersor: Mission ${field} must be a non-empty string: ${mission}`);
