...
function createStart(ctx) {
	return defineTool({
		...
		async execute(args, exec) {
			...
			const launch = args.launch === void 0 ? void 0 : parseKersorLaunchContract(args.launch);
...
function createSealedWorkflow(ctx) {
	return defineTool({
		...
		async execute(args, exec) {
			...
			const call = workflowCallContract(await readDshWorkflowEnvelope(await canonicalRunDirectory(agent, args.exp_dir)));
...
function apply(ctx) {
	...
	ctx.on("tools/pre-execute", async (exec, next) => {
		...
		if (owned !== void 0) {
			...
			if (exec.name === "bash") {
				...
				if (command !== void 0 && manuallyAdvancesRound(command)) return {
					kind: "deny",
		...
		const experimentDescendant = experimentControllerAncestor(ctx, agent) !== void 0;
	...
	ctx.on("tools/post-execute", async (exec, result, next) => {
		...
		if (exec.name === "workflow" && experimentControllerAncestor(ctx, exec.agent) !== void 0) {
			try {
