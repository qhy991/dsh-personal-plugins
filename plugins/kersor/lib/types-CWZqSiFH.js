...
function launchRecord(value, label) {
	...
	if (prototype !== null && prototype !== Object.prototype) throw new TypeError(`${label} must be a plain JSON object`);
	const record = value;
	const unknown = Object.keys(record).find((key) => !LAUNCH_KEYS.has(key));
	if (unknown !== void 0) throw new TypeError(`${label} has unknown field ${JSON.stringify(unknown)}`);
	for (const key of LAUNCH_KEYS) if (!Object.hasOwn(record, key)) throw new TypeError(`${label}.${key} is required`);
	return record;
}
function launchText(record, key, label) {
	const value = record[key];
	if (typeof value !== "string" || value.trim().length === 0) throw new TypeError(`${label}.${key} must be a non-empty string`);
	return value;
}
...
function launchEnum(record, key, values, label) {
	const value = record[key];
	if (typeof value !== "string" || !values.includes(value)) throw new TypeError(`${label}.${key} must be one of ${values.join(", ")}`);
	return value;
}
function launchPositiveNumber(record, key, label) {
	const value = record[key];
	if (typeof value !== "number" || !Number.isFinite(value) || value <= 0) throw new TypeError(`${label}.${key} must be a positive finite number`);
	return value;
}
function launchInteger(record, key, minimum, label) {
	const value = record[key];
	if (!Number.isSafeInteger(value) || value < minimum) throw new TypeError(`${label}.${key} must be a safe integer greater than or equal to ${minimum}`);
	return value;
}
/**
...
function parseKersorLaunchContract(value, label = "KerSor launch contract") {
	...
	return {
		...
		transfer_mode: launchEnum(record, "transfer_mode", [
			"full",
			"measured-only",
			"off"
		], label),
