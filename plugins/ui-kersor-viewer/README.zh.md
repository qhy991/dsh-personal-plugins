...
**一个 store，一个 Host 快照。** 视图数据存在一个 `useSyncExternalStore` observable 里。首次加载、视图挂载和重连会读取 `kersorViewer/snapshot`；之后替换式 `kersor/event` 帧更新同一份原子 projection。后台根扫描会保留最近一次成功内容，不会重新进入可见 loading。选择 run 时读取 `runBacklog` 获取折叠详情；展开经典 Session 时读取 `classicSessionDetail`，并在保持选中期间随替换快照刷新。API Remotes assembly 是生成 contribution 生命周期的唯一 owner；本 UI 只消费已组装的 namespace，不会再次挂载。launcher 发现会先检查 `pluginInventory/list`，再决定是否调用 `kersor/listTasks` 或 `listActive`，因此只读 profile 不会探测缺失的 launcher route。Autonomous run 与带 runtime 事件的经典 `run-N` 目录进入同一条 phase/call fold。横向执行图展示 phase 顺序与并行调用；从 `output.json` 提取的有界字段展示候选集合、选中候选、预期 cycles 以及实测／预估 speedup，同时不暴露候选源码或任意 report 文本。详细 phase 行保留状态、耗时、tokens 与回滚标记；循环重访的 phase 按执行顺序各占一个桶，KSearch 循环读起来就是独立的多轮。
