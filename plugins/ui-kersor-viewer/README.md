...
KerSor activity surfaces, browser half: one sidebar panel shows recent classic/Session-v2 optimization summaries from the host package [`@deepseek-ai/dsh-kersor-viewer`](../kersor-viewer/README.md), then lists autonomous-workflow runs and renders the selected run's live phase/call progress. The compact two-column Session cards show advisory health, canonical phase, last activity, round budget, best/target speedup, backend/mode, selected Workflow, storage kind, and status-warning count. The section header reports both the recent inventory size and the genuinely active count, so stale `optimizing` projections no longer light the global activity dot.
