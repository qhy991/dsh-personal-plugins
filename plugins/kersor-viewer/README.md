...
- `roots` — extra `.kersor` directories whose direct children are KerSor Sessions or direct general Task runs, scanned in addition to registered DSH Workspaces, persisted Session cwd values, and the defaults.
...
When present, summary `workflow_status` owns the displayed Workflow lifecycle and overrides legacy `status`. Thus `{ "status": "completed", "workflow_status": "waiting" }` remains “Waiting to resume” even when the event stream ends with `workflow.completed`: writing the summary settles ingestion and backfill, while `waiting` remains semantically resumable. Canonical task-v1 `succeeded` also projects as completed, whereas `blocked`, `stagnated`, `exhausted`, `failed`, or `error` stay unsuccessful instead of letting a Host-invocation `workflow.completed` event repaint the Mission as completed. When no valid summary exists, a final complete `workflow.completed` or `workflow.failed` ledger event supplies the terminal fallback, so an interrupted direct Task run cannot remain active indefinitely. During an explicit same-run resume the old waiting summary remains the display boundary; once its atomic generation changes, the service rebuilds phases, totals, and bounded results from the complete event ledger so it neither folds a partial append nor double-counts the prefix. A completed Workflow does not make its parent optimization Session terminal; the browser joins the run with the canonical Session projection and shows outstanding Host measurement or decision stages separately.
...
| `src/scanner.ts` | Root scanning: session-v2 run families and direct general Task runs |
