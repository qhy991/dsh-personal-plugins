...
function stemOf(call: KersorCallView): string {
...
async function readBoundedJson(file: string): Promise<Record<string, unknown> | undefined> {
  try {
...
function usageOf(result: Record<string, unknown> | undefined): KersorCallUsageView | undefined {
 ...
 */
export async function readCallDetail(
  runDir: string,
  call: KersorCallView,
): Promise<KersorCallDetailView | undefined> {
  ...
  const modelRole = result === undefined || result.model_role === null
    ? result?.model_role as null | undefined
