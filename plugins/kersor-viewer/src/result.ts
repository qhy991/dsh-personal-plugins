...
function failureKind(value: unknown): 'correctness' | 'benchmark' | 'infrastructure' {
...
async function readObject(file: string, maxBytes: number): Promise<Record<string, unknown> | undefined> {
  try {
...
export async function readWorkflowResult(runDir: string): Promise<KersorWorkflowResultView | undefined> {
  ...
  try {
    ...
    const hostMetric = host?.verdict === 'pass' && host.metric !== null
      ...
      ? host.metric as Record<string, unknown>
      : undefined
    const verification = host?.verdict === 'pass'
      ? 'passed' as const
      : host?.verdict === 'fail' ? 'failed' as const : undefined
