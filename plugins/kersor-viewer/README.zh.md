...
KerSor 始终是唯一状态所有者。bridge 导入 KerSor 规范的 `SessionStore` 与 `AttemptResultStore`；TypeScript 包不重新实现 legacy frontmatter 解析。若 preset 未安装，经典 Session 清单会安静地不可用，autonomous run 发现仍继续工作。
