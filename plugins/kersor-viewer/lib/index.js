...
function isClassicSession(value) {
	...
	const row = value;
	return typeof row.session_id === "string" && typeof row.session_dir === "string" && (row.storage_kind === "v2" || row.storage_kind === "legacy") && (row.lifecycle === "active" || row.lifecycle === "completed" || row.lifecycle === "stalled" || row.lifecycle === "cancelled") && (row.health === "active" || row.health === "stale" || row.health === "needs_resume" || row.health === "terminal" || row.health === "unknown") && (row.status === "terminal-complete" || row.status === "terminal-stalled" || row.status === "terminal-cancelled" || row.status === "resumable" || row.status === "in-progress" || row.status === "pre-round-1") && (row.baseline_next_action == null || row.baseline_next_action === "init" || row.baseline_next_action === "record_verify" || row.baseline_next_action === "new_session") && (row.baseline_reason == null || typeof row.baseline_reason === "string") && (row.profile_reason == null || typeof row.profile_reason === "string") && Array.isArray(row.warnings) && row.warnings.every((item) => typeof item === "string");
}
